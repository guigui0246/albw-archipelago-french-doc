...
- A decrypted, North American A Link Between Worlds `.3ds` ROM until 0.1.3 and `.cci` ROM since 0.1.4. Instructions for dumping your ROM can be found [here](https://wiki.hacks.guide/wiki/3DS:Dump_titles_and_game_cartridges). **Make sure to select "decrypt" when dumping.** Note: if you need a `.cci` ROM, you can rename a `.3ds` ROM and vice versa. These files are identical, only the extension is different.
